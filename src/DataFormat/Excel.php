...
use \PHPExcel_IOFactory as PHPExcelIOFactory;
...
class Excel
{
      ...
      foreach ($sheet->getRowIterator() AS $rowData) {
        ...
        if ($row == 0 && $useHeaders) {
          ...
          foreach ($cellIterator as $cell) {
            if (!is_null($cell)) {
              ...
              if ($headersInLowCase) {
                ...
                $headers[] = strtolower($cell->getCalculatedValue());
              }
            }
          }
          ...
          foreach ($cellIterator as $cell) {
            ...
            if ($this->useHeaders){
              if ($headersInLowCase){
                $ligne[strtolower($headers[$col])] = $value;
              } else {
  ...
  protected function getExcelObjectFromData()
  {
    ...
    if ($this->useHeaders) {
      ...
      foreach ($entetes AS $e) {
        $sheet->setCellValue(static::getAlphaColumnFromIndex($row).$currentLine, $e);
        $row++;
      }
      $currentLine++;
    }
    ...
    foreach ($this->data AS $d) {
      $row = 0;
      foreach ($d AS $k => $v) {
        $sheet->setCellValue(static::getAlphaColumnFromIndex($row).$currentLine, $v);
        $row++;
      }
      $currentLine++;
    }
  ...
  protected static function getAlphaColumnFromIndex($num)
  {
    ...
    $num2 = intval($num / 26);
    if ($num2 > 0) {
      return static::getAlphaColumnFromIndex($num2 - 1) . $letter;
    }
    return $letter;
  }
